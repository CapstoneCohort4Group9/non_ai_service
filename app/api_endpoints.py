...
async def send_itinerary_email(request: SendItineraryEmailRequest, db = Depends(get_sync_db)):
...
async def schedule_callback(request: ScheduleCallbackRequest, db = Depends(get_sync_db)):
...
async def check_refund_eligibility(request: CheckRefundEligibilityRequest, db = Depends(get_sync_db)):
